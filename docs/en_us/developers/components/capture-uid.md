...
Scene initialization captures the UID once per run and writes the default pseudonymous hash to
`MapNavigatorAccountIdentity.attach.account_id`. This Resource-scoped hidden node shares the current account
...
8. **Publish Account Identity** — The Custom Action writes the hash to `MapNavigatorAccountIdentity` so zipline planning can filter by the current account. An `"unknown"` capture publishes an empty value and planning falls back to walking.
...
| MapNavigator | `assets/resource/pipeline/MapNavigator/AccountIdentity.json` | Resource hidden node | Automatically select zipline records for the current account |
