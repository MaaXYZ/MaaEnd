...
- 多账号滑索：官方地图导入从 `/map/mark/list` 的 `roleId` 生成伪匿名账号标识，游戏运行时由场景初始化捕获 UID 并生成相同标识。规划器先按账号、再按地图筛选；旧版没有账号字段的记录不会自动归给当前账号，需重新导入。离线路线预览无法读取游戏 UID，因此只预览最近一次导入的账号；日志分析则按对应运行日志中的账号标识过滤快照。
