...
- 多账号滑索：官方地图导入从 `/map/mark/list` 的 `roleId` 生成伪匿名账号标识，游戏运行时由场景初始化捕获 UID 并生成相同标识。规划器先按账号、再按地图筛选；旧版没有账号字段的记录不会自动归给当前账号，需重新导入。离线路线预览无法读取游戏 UID，可在「规划账号」中选择导入过的伪匿名账号，默认使用最近一次导入并在浏览器中记忆选择；该选择同时用于当前安装目录的滑索架图层，但不会写入复制参数或覆盖实机运行时账号。日志分析仍按对应运行日志中的账号标识过滤快照。
