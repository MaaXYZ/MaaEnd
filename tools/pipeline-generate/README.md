...
MaaEnd 使用五份由 zmdmap 数据 CI 生成并发布的精简游戏数据：
...
- `data/collect_points.json`：篝火与附近采集物的底图坐标。

这些文件都是按任务需要裁剪出的游戏数据，不是完整的上游数据快照，也不包含数据生产端的版本或来源元数据。MaaEnd 通过 `data/version.txt` 单独记录当前数据版本，`fetch-data.mjs` 根据 zmdmap 版本接口下载五份文件；版本未变化且本地缓存完整时跳过下载，网络检查失败时保留当前缓存。
...
完整生成某个任务时使用对应的 pnpm 命令。五个命令都会先同步 zmdmap 精简游戏数据，再执行当前任务自己的生成链路：
...
`data/scripts/` 中的五个 Python 入口用于从本地解包仓库生成精简游戏数据，与
`endfield-workflow` 中的 CI 生成逻辑保持一致。默认目录按当前 MaaEnd 仓库的兄弟目录解析：

- `../BeyondTableCfg/TableCfg`；
- `../BeyondMemoryPack/JsonData`；
- 送货点和采集点坐标另外读取 MaaEnd 当前 `assets/resource/model/map/navmesh/base.nav.gz`。
...
uv run tools/pipeline-generate/data/scripts/collect_points_data.py
```

仓库不在默认相对位置时，可用 `--table-cfg-dir` 覆盖 TableCfg；送货点和采集点脚本还可用
`--gameplay-config-dir`、`--level-data-dir` 和 `--nav` 分别覆盖 BeyondMemoryPack 与
BaseNav 路径。五个入口都支持 `--output`，可将结果写入临时目录做本地/CI 等价校验。
生成结果与已有文件相同时跳过写入；`--force` 可强制重写。
`data/scripts/tablecfg_utils.py` 负责本地仓库路径、远端 JSON 下载、五语言文本引用、结果
比较和 JSON 写入，`data/scripts/navzone_utils.py` 负责 BaseNav 区表读取与世界坐标投影，
任务业务规则分别留在五个入口中。
